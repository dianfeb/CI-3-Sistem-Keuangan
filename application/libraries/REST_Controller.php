...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        {
            if ( ! ($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->where('status', 'Aktif')->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }
    ...
    protected function _check_access()
    {
        ...
        //check if the key has ignore limits
        $accessLimitRow = $this->rest->db
        ->where('api_key', $this->rest->key)
        ->where('ignore_limits', 1)
        ->get($this->config->item('rest_keys_table'))->row_array();

        if (!empty($accessRow) && (!empty($accessRow['all_access']) || count($accessLimitRow)>0))
        {
        	return TRUE;
        }
